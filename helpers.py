...
def update_or_insert_asset(user_id, symbol, quantity, asset_type):
    db = get_db_connection()
    ...
    price = get_crypto_price(symbol) if asset_type == "crypto" else get_stock_price(symbol)
    if not price:
        return False
    ...
    if existing:
        ...
        db.execute("""
            ...
            VALUES (?, ?, ?, ?, ?)
        """, (user_id, symbol, quantity, price, asset_type))
